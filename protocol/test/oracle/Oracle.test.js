...
function uint112s(time, priceNum=1, priceDen=1) {
  return new BN(priceNum).mul(new BN(2).pow(new BN(112))).divn(priceDen).muln(time)
}
...
async function simulateTrade(amm, rsfd, dai) {
  return await amm.simulateTrade(
    new BN(rsfd).mul(new BN(10).pow(new BN(18))),
